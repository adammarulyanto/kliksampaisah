            ...
            <?php endforeach; ?>
              <label class="tpl-card-buy" onClick="window.location='<?=base_url('template')?>'">
                <div class="tpl-thumb-buy">
                    <div class="tpl-thumb-text-buy"><i class="fa-solid fa-plus"></i> <br> <span class="tpl-thumb-text-buy-beli">Beli Template</span> </div>
                </div>
              </label>
          </div>
        </div>
            ...
            <div class="event-block" data-index="0">
              ...
              <div class="field-grid">
                ...
                <div class="field">
                  ...
                  <input type="date" name="acara[0][tanggal]" class="acara-date" required/>
                </div>
                <div class="field">
                  <label>Hari</label>
...
(function () {
  ...
  function renderSummary() {
    ...
    document.querySelectorAll('#acaraList .event-block').forEach((block, i) => {
      ...
      // ← TIDAK ada gallery count di sini
    });
    ...
    // ✅ Gallery count — baca dari window.galleryFiles, BUKAN galleryInput.files
    const galleryCount = (window.galleryFiles || []).length;
    const sumMediaSection = document.getElementById('sumMusik').closest('.sum-section');
    // Hapus row gallery lama sebelum re-render (hindari duplikat)
    sumMediaSection.querySelectorAll('.sum-row-gallery').forEach(el => el.remove());
    const galleryRow = document.createElement('div');
    galleryRow.className = 'sum-row sum-row-gallery';
    galleryRow.innerHTML = `
      <span class="sum-key">Foto Galeri</span>
      <span class="sum-val">${galleryCount > 0 ? galleryCount + ' foto dipilih' : '—'}</span>
    `;
    sumMediaSection.appendChild(galleryRow);
  }
...
<script>
  // ════════════════════════════════
//  TOAST & VALIDASI PER STEP
// ════════════════════════════════
(function () {

  // ─ Toast helper ─
  function showToast(type, title, items = []) {
    ...
    const listHtml = items.length
      ? '<ul class="toast-list">' + items.map(i => `<li>${i}</li>`).join('') + '</ul>'
      : '';
    toast.innerHTML = `
      <div class="toast-icon">${icon}</div>
      <div class="toast-body">
        <p class="toast-title">${title}</p>${listHtml}
      </div>
  ...
  function markError(el) {
    ...
    el.addEventListener('input', () => wrap?.classList.remove('field-error'), { once: true });
    el.addEventListener('change', () => wrap?.classList.remove('field-error'), { once: true });
  }
  ...
  function validateStep(stepIndex) {
    ...
    if (stepIndex === 0) {
      // Step 1: template harus dipilih
      const selected = document.querySelector('input[name="template_id"]:checked');
      if (!selected) {
        errors.push({ label: 'Template belum dipilih', el: document.querySelector('.tpl-grid') });
      }
    }
    ...
    if (stepIndex === 1) {
      ...
      if (!urlEl.value.trim()) {
        ...
        errors.push({ label: 'URL Undangan tidak valid atau sudah dipakai', el: urlEl });
      } else if (urlError && urlError.style.display !== 'none' && urlError.textContent.trim() !== '') {
        // urlError sedang tampil (misal: "URL sudah digunakan" dari server check)
        errors.push({ label: urlError.textContent.trim().replace(/^[✗✕]\s*/, ''), el: urlEl });
      }
    }

    if (stepIndex === 2) {
      // Step 3: nama pengantin
      const fields = [
        { id: 'groomName', label: 'Nama Pengantin Pria' },
        { id: 'brideName', label: 'Nama Pengantin Wanita' },
      ];
      fields.forEach(f => {
        const el = document.getElementById(f.id);
        if (el && !el.value.trim()) errors.push({ label: f.label + ' wajib diisi', el });
      });
    }

    if (stepIndex === 3) {
      // Step 4: setiap acara harus punya nama, tanggal, tempat
      document.querySelectorAll('#acaraList .event-block').forEach((block, i) => {
        const namaEl  = block.querySelector(`[name="acara[${i}][nama]"]`);
        const tglEl   = block.querySelector(`[name="acara[${i}][tanggal]"]`);
        const tempatEl= block.querySelector(`[name="acara[${i}][tempat]"]`);
        if (namaEl && !namaEl.value.trim())   errors.push({ label: `Acara ${i+1}: Nama acara wajib diisi`, el: namaEl });
        if (tglEl  && !tglEl.value.trim())    errors.push({ label: `Acara ${i+1}: Tanggal wajib diisi`, el: tglEl });
        if (tempatEl && !tempatEl.value.trim()) errors.push({ label: `Acara ${i+1}: Nama tempat wajib diisi`, el: tempatEl });
      });
    }

    // Step 4 (Cerita) & 5 (Media) tidak ada field wajib bintang, skip
    return errors;
  }

  // ─ Hook ke btnNext ─
  const origNext = document.getElementById('wzNext');
  if (!origNext) return;

  origNext.addEventListener('click', function (e) {
    // Ambil step saat ini dari wizard (pakai closure-nya sendiri via class .active)
    const activePage = document.querySelector('.wz-page.active');
    if (!activePage) return;
    const stepIndex = parseInt(activePage.dataset.page);

    // Kalau step terakhir (ringkasan), biarkan wizard submit sendiri
    if (stepIndex === 6) return;

    const errors = validateStep(stepIndex);
    if (errors.length === 0) return; // valid → wizard lanjut normal

    // Ada error → stop propagasi ke listener wizard
    e.stopImmediatePropagation();

    errors.forEach(err => {
      if (err.el && err.el.tagName && err.el.tagName !== 'DIV') markError(err.el);
    });

    showToast('error',
      `${errors.length} field wajib belum diisi`,
      errors.map(e => e.label)
    );
  }, true); // useCapture = true → jalan SEBELUM listener wizard
